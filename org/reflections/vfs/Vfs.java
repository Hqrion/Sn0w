...
public abstract class Vfs {
    ...
    public static Dir fromURL(URL url, List<UrlType> urlTypes) {
        for (UrlType type2 : urlTypes) {
            try {
                Dir dir;
                if (!type2.matches(url) || (dir = type2.createDir(url)) == null) continue;
                return dir;
            }
            catch (Throwable e) {
                if (Reflections.log == null) continue;
                Reflections.log.warn("could not create Dir using " + type2 + " from url " + url.toExternalForm() + ". skipping.", e);
            }
        }
