...
implements IObfuscationDataProvider {
    ...
    @Override
    public ObfuscationData<String> getObfClass(TypeHandle type2) {
        return this.getObfClass(type2.getName());
    }
    ...
    private static <T> ObfuscationData<T> applyParents(ObfuscationData<String> parents, ObfuscationData<T> members) {
        for (ObfuscationType type2 : members) {
            String obfClass = parents.get(type2);
            T obfMember = members.get(type2);
            members.put(type2, MemberInfo.fromMapping((IMapping)obfMember).move(obfClass).asMapping());
        }
        return members;
    }
}
