...
extends AnnotatedMixinElementHandler {
    ...
    public void registerInjectionPoint(AnnotatedElementInjectionPoint elem, String format) {
        ...
        String type2 = InjectionPointData.parseType((String)elem.getAt().getValue("value"));
        String target = (String)elem.getAt().getValue("target");
        if ("NEW".equals(type2)) {
            this.remapNewTarget(String.format(format, type2 + ".<target>"), target, elem);
            this.remapNewTarget(String.format(format, type2 + ".args[class]"), elem.getAtArg("class"), elem);
        } else {
            this.remapReference(String.format(format, type2 + ".<target>"), target, elem);
        }
    }
    ...
    protected final void remapNewTarget(String subject, String reference, AnnotatedElementInjectionPoint elem) {
        ...
        if (target != null) {
            ...
            for (ObfuscationType type2 : remapped) {
                MappingMethod mapping = remapped.get(type2);
                if (desc == null) {
                    mappings.put(type2, mapping.getOwner());
                    continue;
                }
                mappings.put(type2, mapping.getDesc().replace(")V", ")L" + mapping.getOwner() + ";"));
            }
