...
class AnnotatedMixin {
    ...
    public AnnotatedMixin(IMixinAnnotationProcessor ap, TypeElement type2) {
        ...
        this.classRef = TypeUtils.getInternalName(type2);
    ...
    private TypeHandle initTargets() {
        ...
        try {
            for (TypeMirror target : this.annotation.getList()) {
                type2 = new TypeHandle((DeclaredType)target);
                if (this.targets.contains(type2)) continue;
                this.addTarget(type2);
        ...
        try {
            for (String privateTarget : this.annotation.getList("targets")) {
                type2 = this.typeProvider.getTypeHandle(privateTarget);
                if (this.targets.contains(type2)) continue;
                if (this.virtual) {
                ...
                this.addSoftTarget(type2, privateTarget);
                if (primaryTarget != null) continue;
                primaryTarget = type2;
            }
        }
    ...
    private void addSoftTarget(TypeHandle type2, String reference) {
        ObfuscationData<String> obfClassData = this.obf.getDataProvider().getObfClass(type2);
    ...
    private void addTarget(TypeHandle type2) {
        this.targets.add(type2);
    }
