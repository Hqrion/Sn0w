...
abstract class AnnotatedMixinElementHandler {
    ...
    protected final void addFieldMappings(String mcpName, String mcpSignature, ObfuscationData<MappingField> obfData) {
        for (ObfuscationType type2 : obfData) {
            MappingField obfField = obfData.get(type2);
            this.addFieldMapping(type2, mcpName, obfField.getSimpleName(), mcpSignature, obfField.getDesc());
        }
    }

    protected final void addFieldMapping(ObfuscationType type2, ShadowElementName name, String mcpSignature, String obfSignature) {
        this.addFieldMapping(type2, name.name(), name.obfuscated(), mcpSignature, obfSignature);
    }

    protected final void addFieldMapping(ObfuscationType type2, String mcpName, String obfName, String mcpSignature, String obfSignature) {
        ...
        this.getMappings().addFieldMapping(type2, from, to);
    }

    protected final void addMethodMappings(String mcpName, String mcpSignature, ObfuscationData<MappingMethod> obfData) {
        for (ObfuscationType type2 : obfData) {
            MappingMethod obfMethod = obfData.get(type2);
            this.addMethodMapping(type2, mcpName, obfMethod.getSimpleName(), mcpSignature, obfMethod.getDesc());
        }
    }

    protected final void addMethodMapping(ObfuscationType type2, ShadowElementName name, String mcpSignature, String obfSignature) {
        this.addMethodMapping(type2, name.name(), name.obfuscated(), mcpSignature, obfSignature);
    }

    protected final void addMethodMapping(ObfuscationType type2, String mcpName, String obfName, String mcpSignature, String obfSignature) {
        ...
        this.getMappings().addMethodMapping(type2, from, to);
    }
    ...
    protected final void validateTarget(Element element, AnnotationHandle annotation, AliasedElementName name, String type2) {
        if (element instanceof ExecutableElement) {
            this.validateTargetMethod((ExecutableElement)element, annotation, name, type2, false, false);
        } else if (element instanceof VariableElement) {
            this.validateTargetField((VariableElement)element, annotation, name, type2);
        }
    }

    protected final void validateTargetMethod(ExecutableElement method, AnnotationHandle annotation, AliasedElementName name, String type2, boolean overwrite, boolean merge) {
        ...
        for (TypeHandle target : this.mixin.getTargets()) {
            ...
            if (targetMethod != null) {
                ...
                this.validateMethodVisibility(method, annotation, type2, target, targetMethod);
                continue;
            }
            if (merge) continue;
            this.printMessage(Diagnostic.Kind.WARNING, "Cannot find target for " + type2 + " method in " + target, method, annotation);
        }
    }

    private void validateMethodVisibility(ExecutableElement method, AnnotationHandle annotation, String type2, TypeHandle target, MethodHandle targetMethod) {
        ...
        if (visTarget.ordinal() > visMethod.ordinal()) {
            this.printMessage(Diagnostic.Kind.WARNING, (Object)((Object)visMethod) + " " + type2 + " method cannot reduce " + visibility, method, annotation);
        } else if (visTarget == Visibility.PRIVATE && visMethod.ordinal() > visTarget.ordinal()) {
            this.printMessage(Diagnostic.Kind.WARNING, (Object)((Object)visMethod) + " " + type2 + " method will upgrade " + visibility, method, annotation);
        }
    }

    protected final void validateTargetField(VariableElement field, AnnotationHandle annotation, AliasedElementName name, String type2) {
        ...
        for (TypeHandle target : this.mixin.getTargets()) {
            ...
            this.ap.printMessage(Diagnostic.Kind.WARNING, "Cannot find target for " + type2 + " field in " + target, field, annotation.asMirror());
        }
    }

    protected final void validateReferencedTarget(ExecutableElement method, AnnotationHandle inject, MemberInfo reference, String type2) {
        ...
        for (TypeHandle target : this.mixin.getTargets()) {
            ...
            this.ap.printMessage(Diagnostic.Kind.WARNING, "Cannot find target method for " + type2 + " in " + target, method, inject.asMirror());
        }
    }
    ...
    protected static <T extends IMapping<T>> ObfuscationData<T> stripOwnerData(ObfuscationData<T> data) {
        ...
        for (ObfuscationType type2 : data) {
            IMapping mapping = (IMapping)data.get(type2);
            stripped.put(type2, mapping.move(null));
        }
        return stripped;
    }
    ...
    protected static <T extends IMapping<T>> ObfuscationData<T> stripDescriptors(ObfuscationData<T> data) {
        ...
        for (ObfuscationType type2 : data) {
            IMapping mapping = (IMapping)data.get(type2);
            stripped.put(type2, mapping.transform(null));
        }
        return stripped;
    }
