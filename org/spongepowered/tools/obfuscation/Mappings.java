...
implements IMappingConsumer {
    ...
    @Override
    public IMappingConsumer.MappingSet<MappingField> getFieldMappings(ObfuscationType type2) {
        IMappingConsumer.MappingSet mappings = this.fieldMappings.get(type2);
        return mappings != null ? mappings : new IMappingConsumer.MappingSet();
    }

    @Override
    public IMappingConsumer.MappingSet<MappingMethod> getMethodMappings(ObfuscationType type2) {
        IMappingConsumer.MappingSet mappings = this.methodMappings.get(type2);
        return mappings != null ? mappings : new IMappingConsumer.MappingSet();
    }
    ...
    @Override
    public void addFieldMapping(ObfuscationType type2, MappingField from, MappingField to) {
        IMappingConsumer.MappingSet<MappingField> mappings = this.fieldMappings.get(type2);
        if (mappings == null) {
            mappings = new IMappingConsumer.MappingSet();
            this.fieldMappings.put(type2, mappings);
        }
        mappings.add(new IMappingConsumer.MappingSet.Pair<MappingField>(from, to));
    }

    @Override
    public void addMethodMapping(ObfuscationType type2, MappingMethod from, MappingMethod to) {
        IMappingConsumer.MappingSet<MappingMethod> mappings = this.methodMappings.get(type2);
        if (mappings == null) {
            mappings = new IMappingConsumer.MappingSet();
            this.methodMappings.put(type2, mappings);
        }
    ...
    implements IMappingConsumer {
        ...
        @Override
        public void addFieldMapping(ObfuscationType type2, MappingField from, MappingField to) {
            if (!this.checkForExistingMapping(type2, from, to, this.fields)) {
                this.mappings.addFieldMapping(type2, from, to);
            }
        }

        @Override
        public void addMethodMapping(ObfuscationType type2, MappingMethod from, MappingMethod to) {
            if (!this.checkForExistingMapping(type2, from, to, this.methods)) {
                this.mappings.addMethodMapping(type2, from, to);
            }
        }

        private <TMapping extends IMapping<TMapping>> boolean checkForExistingMapping(ObfuscationType type2, TMapping from, TMapping to, Map<ObfuscationType, Map<TMapping, TMapping>> mappings) throws MappingConflictException {
            IMapping existing;
            Map<TMapping, TMapping> existingMappings = mappings.get(type2);
            if (existingMappings == null) {
                existingMappings = new HashMap<TMapping, TMapping>();
                mappings.put(type2, existingMappings);
            }
        ...
        @Override
        public IMappingConsumer.MappingSet<MappingField> getFieldMappings(ObfuscationType type2) {
            return this.mappings.getFieldMappings(type2);
        }

        @Override
        public IMappingConsumer.MappingSet<MappingMethod> getMethodMappings(ObfuscationType type2) {
            return this.mappings.getMethodMappings(type2);
        }
    }
