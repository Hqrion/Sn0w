...
public class SignaturePrinter {
    ...
    private StringBuilder appendType(StringBuilder sb, Type type2, String name) {
        switch (type2.getSort()) {
            case 9: {
                return SignaturePrinter.appendArraySuffix(this.appendType(sb, type2.getElementType(), name), type2);
            }
            case 10: {
                return this.appendType(sb, type2.getClassName(), name);
            }
        }
        sb.append(SignaturePrinter.getTypeName(type2, false, this.fullyQualified));
        if (name != null) {
    ...
    public static String getTypeName(Type type2, boolean box) {
        return SignaturePrinter.getTypeName(type2, box, false);
    }

    public static String getTypeName(Type type2, boolean box, boolean fullyQualified) {
        switch (type2.getSort()) {
            case 0: {
            ...
            case 9: {
                return SignaturePrinter.getTypeName(type2.getElementType(), box, fullyQualified) + SignaturePrinter.arraySuffix(type2);
            }
            case 10: {
                String typeName = type2.getClassName();
                if (!fullyQualified) {
                    typeName = typeName.substring(typeName.lastIndexOf(46) + 1);
                }
                return typeName;
    ...
    private static String arraySuffix(Type type2) {
        return Strings.repeat((String)"[]", (int)type2.getDimensions());
    }

    private static StringBuilder appendArraySuffix(StringBuilder sb, Type type2) {
