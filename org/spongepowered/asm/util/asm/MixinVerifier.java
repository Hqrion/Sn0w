...
extends SimpleVerifier {
    ...
    protected boolean isInterface(Type type2) {
        ...
        return ClassInfo.forType(type2).isInterface();
    }
    ...
    protected Type getSuperClass(Type type2) {
        ...
        ClassInfo c = ClassInfo.forType(type2).getSuperClass();
        return c == null ? null : Type.getType("L" + c.getName() + ";");
    }

    @Override
    protected boolean isAssignableFrom(Type type2, Type other) {
        ...
        if (this.currentClass != null && type2.equals(this.currentClass)) {
            ...
            return this.isAssignableFrom(type2, this.getSuperClass(other));
        }
        if (this.currentClass != null && other.equals(this.currentClass)) {
            ...
            if (this.currentClassInterfaces != null) {
                for (int i = 0; i < this.currentClassInterfaces.size(); ++i) {
                    ...
                    if (!this.isAssignableFrom(type2, v)) continue;
                    return true;
                }
            }
            return false;
        }
        ClassInfo typeInfo = ClassInfo.forType(type2);
        if (typeInfo == null) {
            return false;
        }
