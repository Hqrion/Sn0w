...
implements IMixinContext {
    ...
    void addAccessorMethod(MethodNode method, Class<? extends Annotation> type2) {
        this.accessors.add(AccessorInfo.of(this, method, type2));
    }
    ...
    public ClassInfo findRealType(ClassInfo mixin) {
        ...
        ClassInfo type2 = this.targetClassInfo.findCorrespondingType(mixin);
        if (type2 == null) {
    ...
    private Object transformConstant(MethodNode method, Iterator<AbstractInsnNode> iter, Object constant) {
        if (constant instanceof Type) {
            ...
            String desc = this.transformDescriptor(type2);
            if (!type2.toString().equals(desc)) {
    ...
    private String transformDescriptor(Type type2) {
        if (type2.getSort() == 11) {
            return this.transformMethodDescriptor(type2.getDescriptor());
        }
        return this.transformSingleDescriptor(type2);
    }

    private String transformSingleDescriptor(Type type2) {
        if (type2.getSort() < 9) {
            return type2.toString();
        }
        return this.transformSingleDescriptor(type2.toString(), false);
    }

    private String transformSingleDescriptor(String desc, boolean isObject) {
        String type2 = desc;
        while (type2.startsWith("[") || type2.startsWith("L")) {
            if (type2.startsWith("[")) {
                type2 = type2.substring(1);
                continue;
            }
            type2 = type2.substring(1, type2.indexOf(";"));
            isObject = true;
        }
        if (!isObject) {
            return desc;
        }
        String innerClassName = (String)this.innerClasses.get((Object)type2);
        if (innerClassName != null) {
            return desc.replace(type2, innerClassName);
        }
        if (this.innerClasses.inverse().containsKey((Object)type2)) {
            return desc;
        }
        ClassInfo typeInfo = ClassInfo.forName(type2);
        if (!typeInfo.isMixin()) {
            return desc;
        }
        return desc.replace(type2, this.findRealType(typeInfo).toString());
    }
