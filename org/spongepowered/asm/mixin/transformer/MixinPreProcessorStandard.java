...
class MixinPreProcessorStandard {
    ...
    protected boolean attachAccessorMethod(MixinTargetContext context, MixinInfo.MixinMethodNode mixinMethod, SpecialMethod type2) {
        AnnotationNode annotation = mixinMethod.getVisibleAnnotation(type2.annotation);
        if (annotation == null) {
            return false;
        }
        String description = (Object)((Object)type2) + " method " + mixinMethod.name;
        ClassInfo.Method method = this.getSpecialMethod(mixinMethod, type2);
        ...
        context.addAccessorMethod(mixinMethod, type2.annotation);
        return true;
    }
    ...
    protected boolean attachSpecialMethod(MixinTargetContext context, MixinInfo.MixinMethodNode mixinMethod, SpecialMethod type2) {
        AnnotationNode annotation = mixinMethod.getVisibleAnnotation(type2.annotation);
        if (annotation == null) {
            return false;
        }
        if (type2.isOverwrite) {
            this.checkMixinNotUnique(mixinMethod, type2);
        }
        ClassInfo.Method method = this.getSpecialMethod(mixinMethod, type2);
        ...
        if (target == null) {
            ...
            if (target == null) {
                throw new InvalidMixinException((IMixinInfo)this.mixin, String.format("%s method %s in %s was not located in the target class %s. %s%s", new Object[]{type2, mixinMethod.name, this.mixin, context.getTarget(), context.getReferenceMapper().getStatus(), MixinPreProcessorStandard.getDynamicInfo(mixinMethod)}));
            }
        ...
        if (!Bytecode.compareFlags(mixinMethod, target, 8)) {
            throw new InvalidMixinException((IMixinInfo)this.mixin, String.format("STATIC modifier of %s method %s in %s does not match the target", new Object[]{type2, mixinMethod.name, this.mixin}));
        }
        this.conformVisibility(context, mixinMethod, type2, target);
        if (!target.name.equals(mixinMethod.name)) {
            if (type2.isOverwrite && (target.access & 2) == 0) {
    ...
    private void conformVisibility(MixinTargetContext context, MixinInfo.MixinMethodNode mixinMethod, SpecialMethod type2, MethodNode target) {
        ...
        String message = String.format("%s %s method %s in %s cannot reduce visibiliy of %s target method", new Object[]{visMethod, type2, mixinMethod.name, this.mixin, visTarget});
        if (type2.isOverwrite && !this.mixin.getParent().conformOverwriteVisibility()) {
    ...
    protected ClassInfo.Method getSpecialMethod(MixinInfo.MixinMethodNode mixinMethod, SpecialMethod type2) {
        ClassInfo.Method method = this.mixin.getClassInfo().findMethod(mixinMethod, 10);
        this.checkMethodNotUnique(method, type2);
        return method;
    }

    protected void checkMethodNotUnique(ClassInfo.Method method, SpecialMethod type2) {
        if (method.isUnique()) {
            throw new InvalidMixinException((IMixinInfo)this.mixin, String.format("%s method %s in %s cannot be @Unique", new Object[]{type2, method.getName(), this.mixin}));
        }
    }

    protected void checkMixinNotUnique(MixinInfo.MixinMethodNode mixinMethod, SpecialMethod type2) {
        if (this.mixin.isUnique()) {
            throw new InvalidMixinException((IMixinInfo)this.mixin, String.format("%s method %s found in a @Unique mixin %s", new Object[]{type2, mixinMethod.name, this.mixin}));
        }
    }
    ...
    protected boolean attachUniqueMethod(MixinTargetContext context, MixinInfo.MixinMethodNode mixinMethod) {
        ...
        String string = type2 = method.isSynthetic() ? "synthetic" : "@Unique";
        if (Bytecode.getVisibility(mixinMethod).ordinal() < Bytecode.Visibility.PUBLIC.ordinal()) {
            ...
            logger.log(this.mixin.getLoggingLevel(), "Renaming {} method {}{} to {} in {}", new Object[]{type2, mixinMethod.name, mixinMethod.desc, uniqueName, this.mixin});
            mixinMethod.name = method.renameTo(uniqueName);
            return false;
        }
        if (this.strictUnique) {
            throw new InvalidMixinException((IMixinInfo)this.mixin, String.format("Method conflict, %s method %s in %s cannot overwrite %s%s in %s", type2, mixinMethod.name, this.mixin, target.name, target.desc, context.getTarget()));
        }
        ...
        if (unique == null || !Annotations.getValue(unique, "silent", Boolean.FALSE).booleanValue()) {
            if (Bytecode.hasFlag(mixinMethod, 64)) {
                try {
                    ...
                    logger.debug("Discarding sythetic bridge method {} in {} because existing method in {} is compatible", new Object[]{type2, mixinMethod.name, this.mixin, context.getTarget()});
                    return true;
                }
            ...
            logger.warn("Discarding {} public method {} in {} because it already exists in {}", new Object[]{type2, mixinMethod.name, this.mixin, context.getTarget()});
            return true;
        }
        context.addMixinMethod(mixinMethod);
        return true;
    ...
    static enum SpecialMethod {
        ...
        private SpecialMethod(boolean isOverwrite, Class<? extends Annotation> type2) {
            this.isOverwrite = isOverwrite;
            this.annotation = type2;
            this.description = "@" + Bytecode.getSimpleName(type2);
        }
