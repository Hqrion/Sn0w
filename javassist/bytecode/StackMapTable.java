...
extends AttributeInfo {
    ...
    public static class Walker {
        ...
        int stackMapFrames(int pos, int nth) throws BadBytecode {
            int type2 = this.info[pos] & 0xFF;
            if (type2 < 64) {
                this.sameFrame(pos, type2);
                ++pos;
            } else if (type2 < 128) {
                pos = this.sameLocals(pos, type2);
            } else {
                if (type2 < 247) {
                ...
                if (type2 == 247) {
                    ...
                    this.chopFrame(pos, offset, 251 - type2);
                    pos += 3;
                } else if (type2 == 251) {
                    int offset = ByteArray.readU16bit(this.info, pos + 1);
                    ...
                    pos = type2 < 255 ? this.appendFrame(pos, type2) : this.fullFrame(pos);
                }
            }
            return pos;
        }
        ...
        private int sameLocals(int pos, int type2) throws BadBytecode {
            int offset;
            int top = pos;
            if (type2 < 128) {
                offset = type2 - 64;
        ...
        private int appendFrame(int pos, int type2) throws BadBytecode {
            int k = type2 - 251;
            int offset = ByteArray.readU16bit(this.info, pos + 1);
            int[] tags = new int[k];
            int[] data = new int[k];
