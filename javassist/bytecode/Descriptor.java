...
public class Descriptor {
    ...
    public static String of(CtClass type2) {
        StringBuffer sbuf = new StringBuffer();
        Descriptor.toDescriptor(sbuf, type2);
        return sbuf.toString();
    }

    private static void toDescriptor(StringBuffer desc, CtClass type2) {
        if (type2.isArray()) {
            desc.append('[');
            try {
                Descriptor.toDescriptor(desc, type2.getComponentType());
            }
            ...
            desc.append('L');
            desc.append(type2.getName().replace('.', '/'));
            desc.append(';');
        }
    }
    ...
    public static String appendParameter(CtClass type2, String descriptor) {
        ...
        Descriptor.toDescriptor(newdesc, type2);
        newdesc.append(descriptor.substring(i));
        return newdesc.toString();
    }

    public static String insertParameter(CtClass type2, String descriptor) {
        if (descriptor.charAt(0) != '(') {
            return descriptor;
        }
        return "(" + Descriptor.of(type2) + descriptor.substring(1);
    }
    ...
    public static CtClass getReturnType(String desc, ClassPool cp) throws NotFoundException {
        ...
        CtClass[] type2 = new CtClass[1];
        Descriptor.toCtClass(cp, desc, i + 1, type2, 0);
        return type2[0];
    }
    ...
    private static int toCtClass(ClassPool cp, String desc, int i, CtClass[] args, int n) throws NotFoundException {
        ...
        if (c == 'L') {
            ...
            CtClass type2 = Descriptor.toPrimitiveClass(c);
            if (type2 == null) {
                return -1;
            }
            i2 = i + 1;
    ...
    static CtClass toPrimitiveClass(char c) {
        ...
        switch (c) {
            case 'Z': {
                type2 = CtClass.booleanType;
                break;
            }
            case 'C': {
                type2 = CtClass.charType;
                break;
            }
            case 'B': {
                type2 = CtClass.byteType;
                break;
            }
            case 'S': {
                type2 = CtClass.shortType;
                break;
            }
            case 'I': {
                type2 = CtClass.intType;
                break;
            }
            case 'J': {
                type2 = CtClass.longType;
                break;
            }
            case 'F': {
                type2 = CtClass.floatType;
                break;
            }
            case 'D': {
                type2 = CtClass.doubleType;
                break;
            }
            case 'V': {
                type2 = CtClass.voidType;
            }
        }
        return type2;
    }
