...
public class Type {
    ...
    public static Type get(CtClass clazz) {
        Type type2 = (Type)prims.get(clazz);
        return type2 != null ? type2 : new Type(clazz);
    }
    ...
    public Type getComponent() {
        ...
        Type type2 = (Type)prims.get(component);
        return type2 != null ? type2 : new Type(component);
    }
    ...
    public boolean isAssignableFrom(Type type2) {
        ...
        if (type2 == UNINIT && this.isReference() || this == UNINIT && type2.isReference()) {
            return true;
        }
        if (type2 instanceof MultiType) {
            return ((MultiType)type2).isAssignableTo(this);
        }
        if (type2 instanceof MultiArrayType) {
            return ((MultiArrayType)type2).isAssignableTo(this);
        }
        if (this.clazz == null || this.clazz.isPrimitive()) {
            return false;
        }
        try {
            return type2.clazz.subtypeOf(this.clazz);
        }
        catch (Exception e) {
            throw new RuntimeException(e);
        }
    ...
    public Type merge(Type type2) {
        ...
        if (!type2.isReference() || !this.isReference()) {
            return BOGUS;
        }
        if (type2 instanceof MultiType) {
            return type2.merge(this);
        }
        if (type2.isArray() && this.isArray()) {
            return this.mergeArray(type2);
        }
        try {
            return this.mergeClasses(type2);
        }
        catch (NotFoundException e) {
            throw new RuntimeException(e);
        }
    ...
    Type getRootComponent(Type type2) {
        while (type2.isArray()) {
            type2 = type2.getComponent();
        }
        return type2;
    }
    ...
    private Type createArray(Type rootComponent, int dims) {
        ...
        try {
            type2 = Type.get(this.getClassPool(rootComponent).get(name));
        }
        catch (NotFoundException e) {
            throw new RuntimeException(e);
        }
    ...
    private Type mergeArray(Type type2) {
        int targetDims;
        Type targetRoot;
        int thisDims;
    ...
    private Type mergeClasses(Type type2) throws NotFoundException {
        CtClass superClass = Type.findCommonSuperClass(this.clazz, type2.clazz);
        if (superClass.getSuperclass() == null) {
            Map interfaces = this.findCommonInterfaces(type2);
        ...
        Map commonDeclared = this.findExclusiveDeclaredInterfaces(type2, superClass);
    ...
    private Map findCommonInterfaces(Type type2) {
        Map typeMap = this.getAllInterfaces(type2.clazz, null);
        Map thisMap = this.getAllInterfaces(this.clazz, null);
        return this.findCommonInterfaces(typeMap, thisMap);
    }

    private Map findExclusiveDeclaredInterfaces(Type type2, CtClass exclude) {
        Map typeMap = this.getDeclaredInterfaces(type2.clazz, null);
