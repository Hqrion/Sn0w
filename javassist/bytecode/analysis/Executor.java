...
implements Opcode {
    ...
    private Type zeroExtend(Type type2) {
        if (type2 == Type.SHORT || type2 == Type.BYTE || type2 == Type.CHAR || type2 == Type.BOOLEAN) {
            return Type.INTEGER;
        }
        return type2;
    }
    ...
    private void evalGetField(int opcode, int index, Frame frame) throws BadBytecode {
        ...
        Type type2 = this.zeroExtend(this.typeFromDesc(desc));
        if (opcode == 180) {
    ...
    private void evalLoad(Type expected, int index, Frame frame, Subroutine subroutine) throws BadBytecode {
        Type type2 = frame.getLocal(index);
        this.verifyAssignable(expected, type2);
        this.simplePush(type2, frame);
        this.access(index, type2, subroutine);
    }
    ...
    private void evalNewArray(int pos, CodeIterator iter, Frame frame) throws BadBytecode {
        ...
        switch (typeInfo) {
            case 4: {
                type2 = this.getType("boolean[]");
                break;
            }
            case 5: {
                type2 = this.getType("char[]");
                break;
            }
            case 8: {
                type2 = this.getType("byte[]");
                break;
            }
            case 9: {
                type2 = this.getType("short[]");
                break;
            }
            case 10: {
                type2 = this.getType("int[]");
                break;
            }
            case 11: {
                type2 = this.getType("long[]");
                break;
            }
            case 6: {
                type2 = this.getType("float[]");
                break;
            }
            case 7: {
                type2 = this.getType("double[]");
                break;
            }
            default: {
    ...
    private void evalNewObjectArray(int pos, CodeIterator iter, Frame frame) throws BadBytecode {
        int dimensions;
        Type type2 = this.resolveClassInfo(this.constPool.getClassInfo(iter.u16bitAt(pos + 1)));
        String name = type2.getCtClass().getName();
        int opcode = iter.byteAt(pos);
        if (opcode == 197) {
    ...
    private void evalStore(Type expected, int index, Frame frame, Subroutine subroutine) throws BadBytecode {
        ...
        if (expected != Type.OBJECT || type2 != Type.RETURN_ADDRESS) {
            this.verifyAssignable(expected, type2);
        }
        this.simpleSetLocal(index, type2, frame);
        this.access(index, type2, subroutine);
    }
    ...
    private Type simplePeek(Frame frame) {
        Type type2 = frame.peek();
        return type2 == Type.TOP ? frame.getStack(frame.getTopIndex() - 1) : type2;
    }

    private Type simplePop(Frame frame) {
        Type type2 = frame.pop();
        return type2 == Type.TOP ? frame.pop() : type2;
    }

    private void simplePush(Type type2, Frame frame) {
        frame.push(type2);
        if (type2.getSize() == 2) {
            frame.push(Type.TOP);
        }
    ...
    private void access(int index, Type type2, Subroutine subroutine) {
        if (subroutine == null) {
            return;
        }
    ...
    private void simpleSetLocal(int index, Type type2, Frame frame) {
        frame.setLocal(index, type2);
        if (type2.getSize() == 2) {
    ...
    private void verifyAssignable(Type expected, Type type2) throws BadBytecode {
        if (!expected.isAssignableFrom(type2)) {
            throw new BadBytecode("Expected type: " + expected + " Got: " + type2 + " [pos = " + this.lastPos + "]");
        }
    }
}
