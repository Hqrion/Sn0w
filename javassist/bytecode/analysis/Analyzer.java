...
implements Opcode {
    ...
    private ExceptionInfo[] buildExceptionInfo(MethodInfo method) {
        ...
        for (int i = 0; i < table.size(); ++i) {
            ...
            try {
                type2 = index == 0 ? Type.THROWABLE : Type.get(classes.get(constPool.getClassInfo(index)));
            }
    ...
    private Frame firstFrame(MethodInfo method, int maxLocals, int maxStack) {
        ...
        for (int i = 0; i < parameters.length; ++i) {
            Type type2 = this.zeroExtend(Type.get(parameters[i]));
            first.setLocal(pos++, type2);
    ...
    private static class ExceptionInfo {
        ...
        private ExceptionInfo(int start, int end, int handler, Type type2) {
            this.start = start;
            this.end = end;
