...
Opcode {
    ...
    public void setMaxLocals(boolean isStatic, CtClass[] params, int locals) {
        ...
        if (params != null) {
            ...
            for (CtClass type2 : params) {
                if (type2 == doubleType || type2 == longType) {
                    locals += 2;
                    continue;
                }
                ++locals;
    ...
    public void addExceptionHandler(int start, int end, int handler, CtClass type2) {
        this.addExceptionHandler(start, end, handler, this.constPool.addClassInfo(type2));
    }

    public void addExceptionHandler(int start, int end, int handler, String type2) {
        this.addExceptionHandler(start, end, handler, this.constPool.addClassInfo(type2));
    }

    public void addExceptionHandler(int start, int end, int handler, int type2) {
        this.tryblocks.add(start, end, handler, type2);
    }
    ...
    public void addConstZero(CtClass type2) {
        if (type2.isPrimitive()) {
            if (type2 == CtClass.longType) {
                this.addOpcode(9);
            } else if (type2 == CtClass.floatType) {
                this.addOpcode(11);
            } else if (type2 == CtClass.doubleType) {
                this.addOpcode(14);
            } else {
                if (type2 == CtClass.voidType) {
     ...
     */
    public int addLoad(int n, CtClass type2) {
        if (!type2.isPrimitive()) {
            this.addAload(n);
            return 1;
        }
        if (type2 == CtClass.booleanType || type2 == CtClass.charType || type2 == CtClass.byteType || type2 == CtClass.shortType || type2 == CtClass.intType) {
            this.addIload(n);
            return 1;
        }
        if (type2 == CtClass.longType) {
        ...
        if (type2 != CtClass.doubleType) throw new RuntimeException("void type?");
        this.addDload(n);
        return 2;
    }
     ...
     * Enabled aggressive block sorting
     */
    public int addStore(int n, CtClass type2) {
        if (!type2.isPrimitive()) {
            this.addAstore(n);
            return 1;
        }
    ...
    public void addGetfield(CtClass c, String name, String type2) {
        this.add(180);
        int ci = this.constPool.addClassInfo(c);
        this.addIndex(this.constPool.addFieldrefInfo(ci, name, type2));
        this.growStack(Descriptor.dataSize(type2) - 1);
    }

    public void addGetfield(String c, String name, String type2) {
        this.add(180);
        int ci = this.constPool.addClassInfo(c);
        this.addIndex(this.constPool.addFieldrefInfo(ci, name, type2));
        this.growStack(Descriptor.dataSize(type2) - 1);
    ...
    public void addGetstatic(CtClass c, String name, String type2) {
        this.add(178);
        int ci = this.constPool.addClassInfo(c);
        this.addIndex(this.constPool.addFieldrefInfo(ci, name, type2));
        this.growStack(Descriptor.dataSize(type2));
    }

    public void addGetstatic(String c, String name, String type2) {
        this.add(178);
        int ci = this.constPool.addClassInfo(c);
        this.addIndex(this.constPool.addFieldrefInfo(ci, name, type2));
        this.growStack(Descriptor.dataSize(type2));
    ...
    public void addReturn(CtClass type2) {
        if (type2 == null) {
            this.addOpcode(177);
        } else if (type2.isPrimitive()) {
            CtPrimitiveType ptype = (CtPrimitiveType)type2;
