...
extends AttributeInfo {
    ...
    public Annotation getAnnotation(String type2) {
        ...
        for (int i = 0; i < annotations.length; ++i) {
            if (!annotations[i].getTypeName().equals(type2)) continue;
    ...
    public void addAnnotation(Annotation annotation) {
        String type2 = annotation.getTypeName();
        Annotation[] annotations = this.getAnnotations();
        for (int i = 0; i < annotations.length; ++i) {
            if (!annotations[i].getTypeName().equals(type2)) continue;
    ...
    public boolean removeAnnotation(String type2) {
        Annotation[] annotations = this.getAnnotations();
        for (int i = 0; i < annotations.length; ++i) {
            if (!annotations[i].getTypeName().equals(type2)) continue;
            Annotation[] newlist = new Annotation[annotations.length - 1];
    ...
    extends Walker {
        ...
        @Override
        int annotation(int pos, int type2, int numPairs) throws Exception {
            this.currentAnno = new Annotation(type2, this.pool);
            return super.annotation(pos, type2, numPairs);
        }
    ...
    static class Walker {
        ...
        final int annotation(int pos) throws Exception {
            int type2 = ByteArray.readU16bit(this.info, pos);
            int numPairs = ByteArray.readU16bit(this.info, pos + 2);
            return this.annotation(pos + 4, type2, numPairs);
        }
