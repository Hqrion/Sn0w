...
implements TypeTag {
    ...
    private int doXLOAD(TypeData type2, byte[] code, int pos) {
        int localVar = code[pos + 1] & 0xFF;
        return this.doXLOAD(localVar, type2);
    }

    private int doXLOAD(int localVar, TypeData type2) {
        this.stackTypes[this.stackTop++] = type2;
        if (type2.is2WordType()) {
    ...
    private int doXSTORE(int pos, byte[] code, TypeData type2) {
        int index = code[pos + 1] & 0xFF;
        return this.doXSTORE(index, type2);
    }

    private int doXSTORE(int index, TypeData type2) {
        --this.stackTop;
        this.localsTypes[index] = type2;
        if (type2.is2WordType()) {
            --this.stackTop;
    ...
    private int doOpcode148_201(int pos, byte[] code, int op) throws BadBytecode {
        switch (op) {
            ...
            case 189: {
                int i = ByteArray.readU16bit(code, pos + 1);
                String type2 = this.cpool.getClassInfo(i).replace('.', '/');
                type2 = type2.charAt(0) == '[' ? "[" + type2 : "[L" + type2 + ";";
                this.stackTypes[this.stackTop - 1] = new TypeData.ClassName(type2);
                return 3;
            }
            case 190: {
            ...
            case 192: {
                int i = ByteArray.readU16bit(code, pos + 1);
                String type3 = this.cpool.getClassInfo(i);
                if (type3.charAt(0) == '[') {
                    type3 = type3.replace('.', '/');
                }
    ...
    private void doWIDE_XLOAD(int pos, byte[] code, TypeData type2) {
        int index = ByteArray.readU16bit(code, pos + 2);
        this.doXLOAD(index, type2);
    }

    private void doWIDE_STORE(int pos, byte[] code, TypeData type2) {
        int index = ByteArray.readU16bit(code, pos + 2);
        this.doXSTORE(index, type2);
    }
    ...
    private int doNEWARRAY(int pos, byte[] code) {
        ...
        this.stackTypes[s] = new TypeData.ClassName(type2);
        return 2;
    }
