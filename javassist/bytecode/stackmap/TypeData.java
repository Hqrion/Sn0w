...
public abstract class TypeData {
    ...
    public static void aastore(TypeData array, TypeData value, ClassPool cp) throws BadBytecode {
        ...
        if (value instanceof AbsTypeVar) {
            if (array instanceof AbsTypeVar) {
                ...
                if (!array.isNullType()) {
                    String type2 = ArrayElement.typeName(array.getName());
                    value.setType(type2, cp);
                }
            } else {
    ...
    extends TypeData {
        ...
        public TypeData getArrayType(int dim) throws NotFoundException {
            ...
            char type2 = this.name.charAt(-dim);
            if (type2 == '[') {
            ...
            if (type2 == TypeTag.DOUBLE.decodedName) {
                return TypeTag.DOUBLE;
            }
            if (type2 == TypeTag.FLOAT.decodedName) {
                return TypeTag.FLOAT;
            }
            if (type2 == TypeTag.LONG.decodedName) {
                return TypeTag.LONG;
            }
    ...
    extends AbsTypeVar {
        ...
        private CtClass fixByUppers(ArrayList users, ClassPool cp, HashSet visited, CtClass type2) throws NotFoundException {
            if (users == null) {
                return type2;
            }
            int size = users.size();
            for (int i = 0; i < size; ++i) {
                ...
                if (t.uppers != null) {
                    ...
                    for (int k = 0; k < s; ++k) {
                        ...
                        if (!cc.subtypeOf(type2)) continue;
                        type2 = cc;
                    }
                }
                type2 = this.fixByUppers(t.usedBy, cp, visited, type2);
            }
            return type2;
        }
    ...
    extends TypeData {
        ...
        public BasicType(String type2, int tag, char decoded) {
            this.name = type2;
            this.typeTag = tag;
