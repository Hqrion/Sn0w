...
extends Expr {
    ...
    public CtClass getType() throws NotFoundException {
        int type2 = this.etable.catchType(this.index);
        if (type2 == 0) {
            return null;
        }
        ConstPool cp = this.getConstPool();
        String name = cp.getClassInfo(type2);
    ...
    public void insertBefore(String src) throws CannotCompileException {
        ...
        try {
            CtClass type2 = this.getType();
            int var = jv.recordVariable(type2, EXCEPTION_NAME);
            jv.recordReturnType(type2, false);
            b.addAstore(var);
            jv.compileStmnt(src);
            b.addAload(var);
