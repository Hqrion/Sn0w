...
extends MemberCodeGen {
    ...
    public int recordReturnType(CtClass type2, String castName, String resultName, SymbolTable tbl) throws CompileError {
        ...
        int locals = varNo + this.recordVar(type2, resultName, varNo, tbl);
    ...
    public int recordVariable(CtClass type2, String varName, SymbolTable tbl) throws CompileError {
        if (varName == null) {
            return -1;
        }
        int varNo = this.getMaxLocals();
        int locals = varNo + this.recordVar(type2, varName, varNo, tbl);
        this.setMaxLocals(locals);
        return varNo;
    }
    ...
    public void recordVariable(String typeDesc, String varName, int varNo, SymbolTable tbl) throws CompileError {
        ...
        int type2 = MemberResolver.descToType(c);
        String cname = null;
        if (type2 == 307) {
            cname = dim == 0 ? typeDesc.substring(1, typeDesc.length() - 1) : typeDesc.substring(dim + 1, typeDesc.length() - 1);
        }
        Declarator decl = new Declarator(type2, cname, dim, varNo, new Symbol(varName));
        tbl.append(varName, decl);
    }
    ...
    protected void compileUnwrapValue(CtClass type2, Bytecode code) throws CompileError {
        ...
        if (type2 instanceof CtPrimitiveType) {
            ...
            code.addCheckcast(type2);
            this.setType(type2);
        }
    }

    public void setType(CtClass type2) throws CompileError {
        this.setType(type2, 0);
    }

    private void setType(CtClass type2, int dim) throws CompileError {
        if (type2.isPrimitive()) {
            ...
            try {
                this.setType(type2.getComponentType(), dim + 1);
            }
            catch (NotFoundException e) {
                throw new CompileError("undefined type: " + type2.getName());
            }
        } else {
            this.exprType = 307;
            this.arrayDim = dim;
            this.className = MemberResolver.javaToJvmName(type2.getName());
        }
    }

    public void doNumCast(CtClass type2) throws CompileError {
