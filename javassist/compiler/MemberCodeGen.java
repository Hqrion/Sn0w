...
extends CodeGen {
    ...
    protected void atTryStmnt(Stmnt st) throws CompileError {
        ...
        while (catchList != null) {
            ...
            CtClass type2 = this.resolver.lookupClassByJvmName(decl.getClassName());
            decl.setClassName(MemberResolver.javaToJvmName(type2.getName()));
            bc.addExceptionHandler(start, end, bc.currentPc(), type2);
            bc.growStack(1);
            bc.addAstore(var);
    ...
    public void atNewArrayExpr(NewExpr expr) throws CompileError {
        ...
        if (size.length() > 1) {
            ...
            this.atMultiNewArray(type2, classname, size);
            return;
        }
        ASTree sizeExpr = size.head();
        this.atNewArrayExpr2(type2, sizeExpr, Declarator.astToClassName(classname, '/'), init);
    }

    private void atNewArrayExpr2(int type2, ASTree sizeExpr, String jvmClassname, ArrayInit init) throws CompileError {
        String elementClass;
        if (init == null) {
            if (sizeExpr == null) {
        ...
        if (init != null) {
            ...
            for (int i = 0; i < s; ++i) {
                ...
                if (!MemberCodeGen.isRefType(type2)) {
                    this.atNumCastExpr(this.exprType, type2);
                }
                this.bytecode.addOpcode(MemberCodeGen.getArrayWriteOp(type2, 0));
                list = list.tail();
            }
        }
        this.exprType = type2;
    ...
    protected void atMultiNewArray(int type2, ASTList classname, ASTList size) throws CompileError {
        String desc;
        ASTree s;
        int dim = size.length();
        int count = 0;
        ...
        if (type2 == 307) {
            ...
            desc = MemberCodeGen.toJvmTypeName(type2, dim);
        }
    ...
    private boolean setFieldType(FieldInfo finfo) throws CompileError {
        String type2 = finfo.getDescriptor();
        int i = 0;
        int dim = 0;
        char c = type2.charAt(i);
        while (c == '[') {
            ++dim;
            c = type2.charAt(++i);
        }
        this.arrayDim = dim;
        this.exprType = MemberResolver.descToType(c);
        this.className = c == 'L' ? type2.substring(i + 1, type2.indexOf(59, i + 1)) : null;
    ...
    private int addFieldrefInfo(CtField f, FieldInfo finfo) {
        ...
        String type2 = finfo.getDescriptor();
        return cp.addFieldrefInfo(ci, name, type2);
    }
