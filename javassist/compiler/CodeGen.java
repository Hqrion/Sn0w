...
TokenId {
    ...
    public static boolean is2word(int type2, int dim) {
        return dim == 0 && (type2 == 312 || type2 == 326);
    }
    ...
    protected static String toJvmTypeName(int type2, int dim) {
        char c = 'I';
        switch (type2) {
            case 301: {
                c = 'Z';
    ...
    protected final void atReturnStmnt2(ASTree result) throws CompileError {
        ...
        if (result == null) {
            ...
            op = this.arrayDim > 0 ? 176 : ((type2 = this.exprType) == 312 ? 175 : (type2 == 317 ? 174 : (type2 == 326 ? 173 : (CodeGen.isRefType(type2) ? 176 : 172))));
        }
    ...
    protected void atAssignCore(Expr expr, int op, ASTree right, int type2, int dim, String cname) throws CompileError {
        if (op == 354 && dim == 0 && type2 == 307) {
            this.atStringPlusEq(expr, type2, dim, cname, right);
        } else {
            right.accept(this);
            if (this.invalidDim(this.exprType, this.arrayDim, this.className, type2, dim, cname, false) || op != 61 && dim > 0) {
            ...
            if (op != 61) {
                ...
                this.atArithBinExpr(expr, token, k, type2);
            }
        }
        if (op != 61 || dim == 0 && !CodeGen.isRefType(type2)) {
            this.atNumCastExpr(this.exprType, type2);
        }
    }

    private void atStringPlusEq(Expr expr, int type2, int dim, String cname, ASTree right) throws CompileError {
        if (!"java/lang/String".equals(cname)) {
            CodeGen.badAssign(expr);
        }
        this.convToString(type2, dim);
        right.accept(this);
    ...
    private void convToString(int type2, int dim) throws CompileError {
        String method = "valueOf";
        if (CodeGen.isRefType(type2) || dim > 0) {
    ...
    protected static boolean isRefType(int type2) {
        return type2 == 307 || type2 == 412;
    }

    private static int typePrecedence(int type2) {
        if (type2 == 312) {
            return 0;
        }
        if (type2 == 317) {
    ...
    static boolean isP_INT(int type2) {
        return CodeGen.typePrecedence(type2) == 3;
    }
    ...
    private String checkCastExpr(CastExpr expr, String name) throws CompileError {
        ...
        int type2 = expr.getType();
        oprand.accept(this);
        int srcType = this.exprType;
        int srcDim = this.arrayDim;
        if (this.invalidDim(srcType, this.arrayDim, this.className, type2, dim, name, true) || srcType == 344 || type2 == 344) {
        ...
        if (dim > 0) {
            return CodeGen.toJvmTypeName(type2, dim);
        }
        return null;
    }
    ...
    public void atExpr(Expr expr) throws CompileError {
        ...
        if (token == 46) {
            ...
            expr.oprand1().accept(this);
            int type2 = CodeGen.typePrecedence(this.exprType);
            if (this.arrayDim > 0) {
    ...
    protected static int getArrayReadOp(int type2, int dim) {
        if (dim > 0) {
            return 50;
        }
        switch (type2) {
    ...
    protected static int getArrayWriteOp(int type2, int dim) {
        if (dim > 0) {
            return 83;
        }
        switch (type2) {
    ...
    public void atIntConst(IntConst i) throws CompileError {
        ...
        long value = i.get();
        int type2 = i.getType();
        if (type2 == 402 || type2 == 401) {
            this.exprType = type2 == 402 ? 324 : 306;
